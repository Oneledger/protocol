package common_action

import (
	"github.com/Oneledger/protocol/data/governance"
	"github.com/Oneledger/protocol/external_apps/common/common_data"

	abci "github.com/tendermint/tendermint/abci/types"

	"github.com/Oneledger/protocol/data/accounts"
	"github.com/Oneledger/protocol/data/balance"
	"github.com/Oneledger/protocol/data/bitcoin"
	"github.com/Oneledger/protocol/data/fees"
	"github.com/Oneledger/protocol/data/jobs"
	"github.com/Oneledger/protocol/data/ons"
	"github.com/Oneledger/protocol/identity"
	"github.com/Oneledger/protocol/log"
	"github.com/Oneledger/protocol/storage"
)

type Context struct {
	Router              Router
	State               *storage.State
	Header              *abci.Header
	Accounts            accounts.Wallet
	Balances            *balance.Store
	Domains             *ons.DomainStore
	FeePool             *fees.Store
	GovernanceStore     *governance.Store
	Currencies          *balance.CurrencySet
	FeeOpt              *fees.FeeOption
	Validators          *identity.ValidatorStore
	Logger              *log.Logger
	JobStore            *jobs.JobStore
	LockScriptStore     *bitcoin.LockScriptStore
	ExtStores           common_data.Router
}

//type Context = external_common.ExternalContext

func NewContext(r Router, header *abci.Header, state *storage.State,
	wallet accounts.Wallet, balances *balance.Store,
	currencies *balance.CurrencySet, feePool *fees.Store, govern *governance.Store,
	validators *identity.ValidatorStore, domains *ons.DomainStore, jobStore *jobs.JobStore,
	lockScriptStore *bitcoin.LockScriptStore, logger *log.Logger,
	extStores common_data.Router) *Context {

	return &Context{
		Router:              r,
		State:               state,
		Header:              header,
		Accounts:            wallet,
		Balances:            balances,
		Domains:             domains,
		FeePool:             feePool,
		GovernanceStore:     govern,
		Currencies:          currencies,
		Validators:          validators,
		Logger:              logger,
		JobStore:            jobStore,
		LockScriptStore:     lockScriptStore,
		ExtStores:           extStores,
	}
}
