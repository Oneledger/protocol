...
type Router interface {
	...
	AddExternalFuncs(txblock, []func(interface{})) error
}
...
func (r ControllerRouter) AddExternalFuncs(t txblock, f []func(interface{})) error {
	for i := 0; i < len(f); i++ {
		err := r.Add(t, f[i])
		if err != nil {
			return errors.Wrap(err, "add external funcs to router failed")
		}
	}
	return nil
}
