...
var (
	...
	evmEvents    = tmquery.MustParse(fmt.Sprintf("%s='%s'", tmtypes.EventTypeKey, tmtypes.EventTx)).String()
...
func (es *EventSystem) handleLogs(ev coretypes.ResultEvent) {
	...
	txResponse, err := rpctypes.DecodeResultData(data.TxResult.Result.Data)
	if err != nil {
		return
	}
	...
	for _, f := range es.index[filters.LogsSubscription] {
		matchedLogs := FilterLogs(txResponse.Logs, f.logsCrit.FromBlock, f.logsCrit.ToBlock, f.logsCrit.Addresses, f.logsCrit.Topics)
...
func (es *EventSystem) handleChainEvent(ev coretypes.ResultEvent) {
	...
	for _, f := range es.index[filters.BlocksSubscription] {
		f.headers <- rpctypes.EthHeaderFromTendermint(nil, data.Header)
	}
}
