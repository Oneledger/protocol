...
func NewPublicFilterAPI(svc rpctypes.EthService) *PublicFilterAPI {
	api := &PublicFilterAPI{
		svc:     svc,
		logger:  log.NewLoggerWithPrefix(os.Stdout, "eth-filters"),
		filters: make(map[rpc.ID]*filter),
		events:  NewEventSystem(svc.GetStateDB(), svc.GetTMClient()),
	}
	...
	go func(logsCh <-chan coretypes.ResultEvent) {
		...
		for {
			...
			case event := <-logsCh:
				...
				logs := rpctypes.GetTxEthLogs(&dataTx.TxResult.Result)
				matchedLogs := FilterLogs(logs, crit.FromBlock, crit.ToBlock, crit.Addresses, crit.Topics)
	...
	go func(eventCh <-chan coretypes.ResultEvent) {
		...
		for {
			...
			case event := <-eventCh:
				...
				logs := rpctypes.GetTxEthLogs(&dataTx.TxResult.Result)
				matchedLogs := FilterLogs(logs, criteria.FromBlock, criteria.ToBlock, criteria.Addresses, criteria.Topics)
...
func (api *PublicFilterAPI) GetLogs(ctx context.Context, crit filters.FilterCriteria) ([]*ethtypes.Log, error) {
	...
	if crit.BlockHash != nil {
		...
		filter = NewBlockFilter(api.svc, *crit.BlockHash, crit.Addresses, crit.Topics)
	} else {
		...
		filter = NewRangeFilter(api.svc, begin, end, crit.Addresses, crit.Topics)
	}

	// Run the filter and return all the logs
	logs, err := filter.Logs(nil)
	if err != nil {
		return nil, err
	}
...
func (api *PublicFilterAPI) GetFilterLogs(ctx context.Context, id rpc.ID) ([]*ethtypes.Log, error) {
	...
	if f.crit.BlockHash != nil {
		// Block filter requested, construct a single-shot filter
		filter = NewBlockFilter(api.svc, *f.crit.BlockHash, f.crit.Addresses, f.crit.Topics)
	} else {
		// Convert the RPC block numbers into internal representations
		begin := rpc.LatestBlockNumber.Int64()
		...
		// Construct the range filter
		filter = NewRangeFilter(api.svc, begin, end, f.crit.Addresses, f.crit.Topics)
	}
	// Run the filter and return all the logs
	logs, err := filter.Logs(nil)
	if err != nil {
