...
var (
	...
	// olvm type
	olvmType = new(olvm.Transaction).Type().String()
	// listen only olvm txs
	olvmEvents = tmquery.MustParse(fmt.Sprintf("%s='%s' AND olvm.tx.type='%s'", tmtypes.EventTypeKey, tmtypes.EventTx, olvmType)).String()
	...
	headerEvents = tmtypes.QueryForEvent(tmtypes.EventNewBlock).String()
...
func NewEventSystem(eventBus *tmtypes.EventBus) *EventSystem {
	...
	es := &EventSystem{
		...
		logger:        log.NewLoggerWithPrefix(os.Stdout, "filter_system"),
		eventBus:      eventBus,
		index:         index,
...
func (es *EventSystem) unsubscribe(subscriber, query string) error {
	q, err := tmquery.New(query)
	if err != nil {
		return errors.Wrap(err, "failed to parse query")
	}
	return es.eventBus.Unsubscribe(es.ctx, subscriber, q)
}
...
func (es EventSystem) SubscribeNewHeads() (*Subscription, context.CancelFunc, error) {
	sub := &Subscription{
		...
		event:     headerEvents,
		created:   time.Now().UTC(),
		headers:   make(chan *rpctypes.Header),
		installed: make(chan struct{}, 1),
		err:       make(chan error, 1),
	}
	return es.subscribe(sub)
...
func (es *EventSystem) handleLogs(ev coretypes.ResultEvent) {
	...
	for _, f := range es.index[filters.LogsSubscription] {
		matchedLogs := FilterLogs(logReceipt.Logs, f.logsCrit.FromBlock, f.logsCrit.ToBlock, f.logsCrit.Addresses, f.logsCrit.Topics)
...
func (es *EventSystem) handleChainEvent(ev coretypes.ResultEvent) {
	...
	for _, f := range es.index[filters.BlocksSubscription] {
		header, err := onetypes.EthHeaderFromTendermint(data.Block)
		if err != nil {
			continue
		}
		f.headers <- header
