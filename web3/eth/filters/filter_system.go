...
func NewEventSystem(stateDB *vm.CommitStateDB, client rpcclient.Client) *EventSystem {
...
func (es *EventSystem) subscribe(sub *Subscription) (*Subscription, context.CancelFunc, error) {
	...
	case
		...
		eventCh, err = es.client.Subscribe(es.ctx, string(sub.id), sub.event, []int{tmChannelCapacity}...)
	default:
...
func (es *EventSystem) handleLogs(ev coretypes.ResultEvent) {
	...
	logs := onetypes.GetTxEthLogs(&data.TxResult.Result)
