...
func NewAPI(clientCtx rpctypes.Web3Context) *PubSubAPI {
	return &PubSubAPI{
		clientCtx: clientCtx,
		events:    rpcfilters.NewEventSystem(clientCtx.GetAPI().RPCClient()),
	...
	go func(headersCh <-chan coretypes.ResultEvent, errCh <-chan error) {
		for {
			...
			case event := <-headersCh:
				...
				header := rpctypes.EthHeaderFromTendermint(api.clientCtx.GetContractStore(), data.Header)
	...
	go func(ch <-chan coretypes.ResultEvent, errCh <-chan error) {
		for {
			...
			case event := <-ch:
				...
				var resultData rpctypes.ResultData
				resultData, err = rpctypes.DecodeResultData(dataTx.TxResult.Result.Data)
				if err != nil {
					return
				}
