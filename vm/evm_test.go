...
func assemblyCtxData(acc accounts.Account, currencyName string, currencyDecimal int, setStore bool, setLogger bool, setCoin bool, setCoinAddr crypto.Address) *TestContext {
	ctx := &TestContext{}
...
func getNonce(ctx *TestContext, from keys.Address) (nonce uint64) {
...
func TestRunner_TokenMapWithDeleteFunctionality(t *testing.T) {
	...
	ethAcc, _ := ctx.StateDB.GetAccountKeeper().NewAccountWithAddress(acc.Address())
	ctx.StateDB.GetAccountKeeper().SetAccount(*ethAcc)
	...
	evm := NewEVMTransaction(ctx.StateDB, new(ethcore.GasPool).AddGas(SimulationBlockGasLimit), ctx.Header, acc.Address(), nil, nonce, value, code, nil, gas, DefaultGasPrice, true).NewEVM()
