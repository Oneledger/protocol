...
func (st *DelegationStore) UpdateWithdrawReward(height int64) {
	...
	// st_m_ operation
	fmt.Printf("--- Block mature update at height: %d ---\n", height)
	...
	if len(mature.Data) == 0 {
		fmt.Println("UpdateWithdrawReward: nothing to update")
	} else {
		// reset info
		mature = &MatureBlock{
			Height: height,
		...
		fmt.Println("UpdateWithdrawReward: updated")
	}
	fmt.Println("----------------------------------------------")
}
