...
class ReinvestRewardsTxLoad(TxLoad):
    ...
    def setup(self, interval):
        super(ReinvestRewardsTxLoad, self).setup(interval)
    ...
    def run_tx(self, i):
        if self.balance == 0:
            ...
            self.balance = waitfor_rewards(self.test_account, '10', "balance")
            self.log("rewards distributed: {} OLT".format(self.balance))
        super(ReinvestRewardsTxLoad, self).run_tx(i)
        log = self.tx_invest.send(10**18, exit_on_err=False, mode=TxAsync)
        self.balance -= 1
        if len(log) > 0:
            self.log(log)
