...
func NewService(ctx client.ExtServiceContext, router action.Router, currencies *balance.CurrencySet,
	feePool *fees.Store, domains *ons.DomainStore, govern *governance.Store, delegators *delegation.DelegationStore, evidenceStore *evidence.EvidenceStore, validators *identity.ValidatorStore,
...
func (svc *Service) validateAndSignTx(req client.BroadcastRequest) ([]byte, error) {
	...
	ctx := action.NewContext(svc.router, nil, nil, nil, nil, svc.currencies,
		svc.feePool, svc.validators, nil, svc.domains, svc.delegators, svc.evidenceStore, svc.trackers, nil, nil, nil, svc.logger,
